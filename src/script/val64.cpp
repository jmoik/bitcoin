...
bool Val64::DivMod(Val64& v1, Val64& v2, DivModOp op)
{
    ...
    if (v1.m_size == 0) return true;

    // Avoid the general multi-limb algorithm when both operands fit in one
    // word. The result cannot require more storage than the dividend.
    if (v1.m_limbs.size() == 1 && v2.m_limbs.size() == 1) {
        const uint64_t dividend{v1.Get(0)};
        const uint64_t divisor{v2.Get(0)};
        v1.Set(0, op == DivModOp::DIV ? dividend / divisor : dividend % divisor);
        v1.TrimTail();
        return true;
    }
    ...
    if (n == 1) {
        ...
        for (size_t i{v1.m_limbs.size()}; i > 0; --i) {
            ...
            assert(!result.quotient_is_beta);
