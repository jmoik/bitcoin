...
static inline void popstack(ScriptStack& stack)
{
    if (stack.empty())
        throw std::runtime_error("popstack(ScriptStack& stack): stack empty");
    stack.pop_back();
}
...
void push64(ScriptStack& stack, Val64 &v)
{
    stack.push_back(v.move_to_valtype());
}

static void stack_push_costed(ScriptStack& stack,
...
bool EvalScript(ScriptStack& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror, uint64_t* varops_budget)
{
            ...
            switch (opcode)
            {
                ...
                case OP_TOALTSTACK:
                {
                    if (stack.size() < 1)
                        return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
                    const valtype& vch = stacktop(-1);
                    stack_push_costed(altstack, vch, varcost);
                    popstack(stack);
                }
                break;
                ...
                case OP_FROMALTSTACK:
                {
                    ...
                    const valtype& vch = altstacktop(-1);
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror, uint64_t* varops_budget)
{
    ...
    // Convert from std::vector to ScriptStack
    ...
    // Convert back from ScriptStack to std::vector
    stack.clear();
