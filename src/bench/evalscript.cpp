                            ...
                            const std::vector<unsigned char> &op2)
{
    ...
    bench.warmup(10).unit("ops").run([&] {
        ...
        varops::Budget varops_budget{100'000'000'000'000};

        ValtypeStack valtype_stack{stack};
        if (!EvalTapscriptV2(valtype_stack, script, 0, checker, sdata, varops_budget, &serror)) {
...
// BIP 440: We assume that the manipulation of the stack vector itself (e.g. OP_DROP) is negligible.
