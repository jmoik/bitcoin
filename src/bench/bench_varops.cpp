...
static void RunBenchmark(ankerl::nanobench::Bench& bench, 
    ...
    std::vector<ScriptStack> stack_pool;
    ...
    bench.run(test_case.name, [&] {
        ...
        ScriptStack& working_stack = stack_pool[stack_index];
...
static ScriptStack CreateRandomStack(int num_elements) {
    ScriptStack stack;
...
static std::vector<BenchTestCase> CreateTestCases() {
    ...
    for (const auto& script_template : script_templates) {
        ...
        // Add RANDOM_32000_ELEMENTS version for every operation
        ScriptStack random_stack = CreateRandomStack(32000);
        for (const auto& script_template : script_templates) {
                test_cases.emplace_back(
