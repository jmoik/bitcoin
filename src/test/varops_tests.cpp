...
{
    static_assert(std::is_same_v<decltype(varops::mul_cost(size_t{0}, size_t{0})), uint64_t>);

    BOOST_CHECK_EQUAL(varops::word_size(0), 0);
    BOOST_CHECK_EQUAL(varops::word_size(1), 8);
    BOOST_CHECK_EQUAL(varops::word_size(8), 8);
    BOOST_CHECK_EQUAL(varops::word_size(9), 16);
    ...
    for (const auto& [size_a, size_b] : {
        ...
        BOOST_TEST_CONTEXT("sizes " << size_a << ", " << size_b) {
            BOOST_CHECK_EQUAL(varops::mul_cost(size_a, size_b), ExpectedMulCost(size_a, size_b));
            BOOST_CHECK_EQUAL(varops::div_cost(size_a, size_b), ExpectedDivModCost(size_a, size_b));
            BOOST_CHECK_EQUAL(varops::mod_cost(size_a, size_b), ExpectedDivModCost(size_a, size_b));
        }
    }

    BOOST_CHECK_GT(varops::mul_cost(4'000'000, 4'000'000), uint64_t{std::numeric_limits<uint32_t>::max()});
    BOOST_CHECK_GT(varops::div_cost(4'000'000, 4'000'000), uint64_t{std::numeric_limits<uint32_t>::max()});
}

BOOST_AUTO_TEST_CASE(bip440_and_bip441_word_granular_cost_helpers)
{
    BOOST_CHECK_EQUAL(varops::lengthconv_cost(1), 8 * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::lengthconv_cost(8), 8 * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::lengthconv_cost(9), 16 * varops::COST_FAST);

    BOOST_CHECK_EQUAL(varops::comparingzero_cost(3), 8 * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::comparing_cost(1, 9), 16 * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::booland_cost(1, 9), (8 + 16) * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::boolor_cost(9, 1), (16 + 8) * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::within_cost(1, 9, 17), (16 + 24) * varops::COST_FAST);

    BOOST_CHECK_EQUAL(varops::add_cost(1, 9), 16 * (varops::COST_ARITH + varops::COST_COPYING));
    BOOST_CHECK_EQUAL(varops::sub_cost(9, 1), 16 * varops::COST_ARITH);
    BOOST_CHECK_EQUAL(varops::minmax_cost(1, 9), 16 * varops::COST_OTHER);

    BOOST_CHECK_EQUAL(varops::invert_cost(9), 16 * varops::COST_OTHER);
    BOOST_CHECK_EQUAL(varops::and_cost(1, 9), (8 + 16) * varops::COST_FAST);
    BOOST_CHECK_EQUAL(varops::or_cost(1, 9), 8 * varops::COST_OTHER);
    BOOST_CHECK_EQUAL(varops::xor_cost(9, 1), 8 * varops::COST_OTHER);

    BOOST_CHECK_EQUAL(varops::twomul_cost(9), 16 * (varops::COST_COPYING + varops::COST_OTHER));
    BOOST_CHECK_EQUAL(varops::twodiv_cost(9), 16 * varops::COST_OTHER);
    BOOST_CHECK_EQUAL(varops::upshift_bitshift_cost(9, 0), 16 * varops::COST_OTHER);
    BOOST_CHECK_EQUAL(varops::upshift_bitshift_cost(1, 9), 16 * varops::COST_OTHER);
    BOOST_CHECK_EQUAL(varops::checksigadd_incr_cost(0), 8 * (varops::COST_ARITH + varops::COST_COPYING));
    BOOST_CHECK_EQUAL(varops::checksigadd_incr_cost(9), 16 * (varops::COST_ARITH + varops::COST_COPYING));
}
