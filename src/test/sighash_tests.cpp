...
void static MutateInputs(CMutableTransaction &tx, const bool inPrevout, const bool inInputSequence) {
    ...
    for (std::size_t in = 0; in < tx.vin.size(); in++) {
        ...
        if (inPrevout) {
            txin.prevout.hash = Txid::FromUint256(rng.rand256());
            txin.prevout.n = rng.randbits(2);
        }
...
static std::vector<CTxOut> MutateSpentOutputs(const size_t inNumOutputs, const bool inInputScripts, const bool inValue) {
    ...
    for (std::size_t in = 0; in < spent_outputs.size(); in++) {
        ...
        if (inValue) {
            ...
            spent_outputs[in].nValue = context.randrange(99999999)+1;
        }
    }
...
BOOST_AUTO_TEST_CASE(sighash_anyprevout_legacy)
{
    ...
    for (int i=0; i<nRandomTests; i++) {
        ...
        int nHashType = static_cast<int32_t>(rng.rand32()) & ~SIGHASH_INPUT_MASK;
        ...
        int nIn = rng.randrange(tx.vin.size());
...
BOOST_AUTO_TEST_CASE(sighash_anyprevout_taproot)
{
    ...
    for (int i=0; i<nRandomTests; i++) {
        ...
        execdata.m_annex_present = rng.randbool();
        execdata.m_annex_hash = rng.rand256();
        execdata.m_tapleaf_hash_init = true;
        execdata.m_tapleaf_hash = rng.rand256();
        execdata.m_codeseparator_pos_init = true;
        execdata.m_codeseparator_pos = rng.rand32();
...
BOOST_AUTO_TEST_CASE(sighash_anyprevout_is_not_tapscript_v2)
{
    ...
    for (const uint8_t hash_type : {
             uint8_t{SIGHASH_ALL},
             uint8_t{SIGHASH_ANYPREVOUT | SIGHASH_ALL},
             uint8_t{SIGHASH_ANYPREVOUTANYSCRIPT | SIGHASH_ALL}}) {
        uint256 hash;
