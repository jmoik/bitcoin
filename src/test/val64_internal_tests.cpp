...
BOOST_AUTO_TEST_CASE(val64_div_mod_normalizes_clear_top_bit_divisor)
{
    for (bool portable_math : {false, true}) {
        ...
        for (bool offset_span : {false, true}) {
            ...
            // Cover every normalization shift, with/without a carry limb,
            // single-word division, and both local and heap scratch storage.
            for (size_t words : {1U, 2U, 3U, 4U, 8U, 16U, 17U}) {
                for (unsigned int shift{0}; shift < 64; ++shift) {
                    const cpp_int b{(cpp_int{1} << (64 * words - shift)) - 1};
                    for (size_t extra : {0U, 1U}) {
                        std::vector<unsigned char> dividend(words * 8 + extra, 0xff);
                        std::vector<unsigned char> divisor{cpp_int_to_vector(b)};
                        const cpp_int a{vector_to_cpp_int(dividend)};
                        if (shift % 2) {
                            dividend.resize(dividend.size() + 8);
                            divisor.resize(divisor.size() + 8);
                        }
