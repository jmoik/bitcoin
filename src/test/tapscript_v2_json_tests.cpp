    ...
    const std::vector<std::vector<unsigned char>>& expected_stack)
{
    ...
    for (size_t i = 0; i < std::max(actual_stack.size(), expected_stack.size()); ++i) {
        ...
        if (i < expected_stack.size()) {
            std::cerr << "Expected: ";
            std::ranges::for_each(expected_stack[i], [](unsigned char c) { std::cerr << std::hex << +c << " "; });
        } else {
        ...
        if (i < actual_stack.size()) {
            std::cerr << "Actual: ";
            std::ranges::for_each(actual_stack[i], [](unsigned char c) { std::cerr << std::hex << +c << " "; });
        } else {
            ...
            BOOST_TEST_CONTEXT("category " << category_name << ", test " << full_test_name) {
                try {
                    ...
                    if (success) {
                        ...
                        if (stack.get_stack() != expected_stack) {
                            PrintStackComparison(full_test_name, stack.get_stack(), expected_stack);
                        }
                        BOOST_CHECK_MESSAGE(stack.get_stack() == expected_stack,
