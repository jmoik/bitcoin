...
public:
    ...
    mutable KeyVersion last_keyversion{KeyVersion::TAPROOT};
    ...
    bool CheckSchnorrSignature(Span<const unsigned char>, KeyVersion keyversion, Span<const unsigned char>, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError*) const override
    {
        ++schnorr_calls;
        last_keyversion = keyversion;
...
inline EvalOutcome EvalTapscriptV2WithFlagsAndChecker(const CScript& script, const Stack& initial_stack, script_verify_flags flags, const BaseSignatureChecker& checker, uint64_t budget)
{
    ...
    return {ok, error, varops_budget.Remaining(), stack.get_stack()};
}
