...
static bool Verify(const CScript& scriptSig, const CScript& scriptPubKey, bool fStrict, ScriptError& err)
{
    ...
    return VerifyScript(scriptSig, scriptPubKey, nullptr, fStrict ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE, MutableTransactionSignatureChecker(&txTo, 0, txFrom.vout[0].nValue, MissingDataBehavior::ASSERT_FAIL), &err, varops_budget);
}
...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            std::shared_ptr<varops::Budget> varops_budget{std::make_shared<varops::Budget>(varops::TxBudget(GetTransactionWeight(tx_to)))};
            bool sigOK = !CScriptCheck(txFrom.vout[txTo[i].vin[0].prevout.n], tx_to, signature_cache, 0, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, false, &txdata, varops_budget)().has_value();
            if (i == j)
