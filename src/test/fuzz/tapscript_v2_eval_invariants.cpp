...
public:
    explicit StableChecker(FuzzedDataProvider& provider) : m_signature_result{provider.ConsumeBool()},
                                                           ...
                                                           m_template_result{provider.ConsumeBool()}
    {
    }

    bool CheckSchnorrSignature(Span<const unsigned char>, KeyVersion, Span<const unsigned char>, SigVersion, ScriptExecutionData&, ScriptError*) const override
    {
    ...
    bool CheckDefaultCheckTemplateVerifyHash(const Span<const unsigned char>&) const override
    {
        return m_template_result;
    }
};
...
std::vector<unsigned char> ConsumeStackElement(FuzzedDataProvider& provider)
{
    if (provider.ConsumeIntegralInRange<uint8_t>(0, 31) == 0) {
        ...
        const std::vector<unsigned char> seed{provider.ConsumeBytes<unsigned char>(std::min<size_t>(provider.remaining_bytes(), 16))};
...
{
    return provider.PickValueInArray<opcodetype>({OP_0, OP_1, OP_2, OP_3, OP_NOP, OP_IF, OP_NOTIF, OP_ELSE, OP_ENDIF,
                                                  OP_VERIFY, OP_TOALTSTACK, OP_FROMALTSTACK, OP_DROP, OP_DUP, OP_NIP, OP_OVER,
                                                  OP_PICK, OP_ROLL, OP_ROT, OP_SWAP, OP_TUCK, OP_SIZE, OP_EQUAL, OP_EQUALVERIFY, OP_1ADD, OP_1SUB,
                                                  OP_2MUL, OP_2DIV, OP_INVERT, OP_AND, OP_OR, OP_XOR, OP_ADD, OP_SUB,
                                                  ...
                                                  OP_MIN, OP_MAX, OP_WITHIN, OP_MUL, OP_DIV, OP_MOD, OP_LSHIFT,
                                                  OP_RSHIFT, OP_CAT, OP_SUBSTR, OP_LEFT, OP_RIGHT, OP_RIPEMD160, OP_SHA1,
                                                  OP_SHA256, OP_HASH160, OP_HASH256, OP_CHECKLOCKTIMEVERIFY,
                                                  OP_CHECKSEQUENCEVERIFY, OP_CHECKSIG, OP_CHECKSIGVERIFY, OP_CHECKSIGADD,
                                                  OP_INTERNALKEY, OP_CHECKSIGFROMSTACK, OP_TEMPLATEHASH, OP_CODESEPARATOR});
}

CScript ConsumeGsrScript(FuzzedDataProvider& provider)
{
                              ...
                              uint64_t budget)
{
    ...
    return {
        ...
        stack.get_stack(),
        stack.get_total_size(),
        stack.get_max_element_size(),
    };
}

void AssertRawEqual(const RawEvalOutcome& a, const RawEvalOutcome& b)
{
    Assert(a.ok == b.ok);
    Assert(a.error == b.error);
    Assert(a.remaining_budget == b.remaining_budget);
    Assert(a.stack == b.stack);
    Assert(a.total_stack_size == b.total_stack_size);
    Assert(a.max_element_size == b.max_element_size);
}
...
void CheckScriptRestorationGate(const CScript& leaf_script,
                                const Stack& initial_stack,
                                const BaseSignatureChecker& checker,
                                uint64_t budget)
{
    ...
    const VerifyOutcome strict_rerun{VerifySpend(spend, TAPSCRIPT_V2_SCRIPT_VERIFY_FLAGS, checker, budget)};
    Assert(strict.ok == strict_rerun.ok);
    Assert(strict.error == strict_rerun.error);
    Assert(strict.remaining_budget == strict_rerun.remaining_budget);
}
} // namespace
...
FUZZ_TARGET(tapscript_v2_eval_invariants)
{
    ...
    const CScript script{ConsumeGsrScript(provider)};
    const Stack stack{ConsumeStack(provider)};
    const script_verify_flags flags{script_verify_flags::from_int(provider.ConsumeIntegral<script_verify_flags::value_type>())};
    ...
    const RawEvalOutcome rerun{EvalWithBudget(script, stack, flags, checker, budget)};
    AssertRawEqual(outcome, rerun);
    ...
    CheckScriptRestorationGate(script, stack, checker, budget);
}
