...
public:
    ...
    static void SetForcePortableMath(bool value) { Val64::force_portable_math = value; }
};
...
ActualResult ExecuteArithmetic(ArithmeticOp op, const Bytes& a, const Bytes& b, bool portable_math)
{
    ...
    switch (op) {
    case ArithmeticOp::ADD:
        Val64::op_add(va, vb, cost);
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::SUB:
        if (!Val64::op_sub(va, vb, cost)) return {std::nullopt, cost};
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::ONE_ADD:
        Val64::op_1add(va, cost);
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::ONE_SUB:
        if (!Val64::op_1sub(va, cost)) return {std::nullopt, cost};
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::TWO_MUL:
        Val64::op_2mul(va, cost);
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::TWO_DIV:
        Val64::op_2div(va, cost);
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::MIN:
        Val64::op_min(va, vb, cost);
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::MAX:
        Val64::op_max(va, vb, cost);
        return {va.move_to_valtype(), cost};
    case ArithmeticOp::MUL: {
        Val64 result{Val64::op_mul(va, vb)};
        return {result.move_to_valtype(), std::nullopt};
    }
    case ArithmeticOp::DIV:
        if (!Val64::op_div(va, vb)) return {std::nullopt, std::nullopt};
        return {va.move_to_valtype(), std::nullopt};
    case ArithmeticOp::MOD:
        if (!Val64::op_mod(va, vb)) return {std::nullopt, std::nullopt};
        return {va.move_to_valtype(), std::nullopt};
    }
    Assert(false);
    return {};
...
ActualResult ExecuteBitwiseShift(BitwiseShiftOp op, const Bytes& a, const Bytes& b)
{
    ...
    switch (op) {
    case BitwiseShiftOp::INVERT:
        Val64::op_invert(va, cost);
        break;
    case BitwiseShiftOp::AND:
        Val64::op_and(va, vb, cost);
        break;
    case BitwiseShiftOp::OR:
        Val64::op_or(va, vb, cost);
        break;
    case BitwiseShiftOp::XOR:
        Val64::op_xor(va, vb, cost);
        break;
    case BitwiseShiftOp::UPSHIFT:
        if (!Val64::op_upshift(va, vb, MAX_TAPSCRIPT_V2_STACK_ELEMENT_SIZE, cost)) {
            return {std::nullopt, cost};
        }
        break;
    case BitwiseShiftOp::DOWNSHIFT:
        Val64::op_downshift(va, vb, cost);
        break;
    }
    return {va.move_to_valtype(), cost};
}
...
Bytes ConsumeLargeOperand(FuzzedDataProvider& provider)
{
    ...
    return ConsumeBytesWithSize(provider, ConsumeSize(provider, boundaries, MIN_LARGE_OPERAND_SIZE, MAX_LARGE_OPERAND_SIZE));
}
...
void CheckPredicates(FuzzedDataProvider& provider, const Bytes& a, const Bytes& b)
{
...

    {
        Val64Fuzz va{a};
        uint64_t cost{0};
        Assert(va.to_u64_ceil(max, cost) == expected_ceil);
        Assert(cost == LengthConversionCost(a.size()));
    }
    {
        Val64Fuzz va{a};
        uint64_t cost{0};
        Assert(va.is_zero() == expected_zero);
        Assert(va.is_zero(cost) == expected_zero);
    ...
    }
    {
        ...
        Val64Fuzz vb{b};
        uint64_t cost{0};
        Assert(va.cmp(vb) == expected_cmp);
        Assert(va.cmp(vb, cost) == expected_cmp);
