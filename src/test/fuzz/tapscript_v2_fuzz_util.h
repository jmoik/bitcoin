...
{
    if (bytes.size() > 8 && std::ranges::any_of(bytes.begin() + 8, bytes.end(), [](unsigned char byte) { return byte != 0; })) {
        return maximum;
    }
    uint64_t value{0};
...
inline void CheckExactEval(const CScript& script, const Stack& initial_stack, const Stack& expected_stack, uint64_t expected_cost)
{
    const BaseSignatureChecker checker;
    const EvalOutcome exact{EvalTapscriptV2WithFlagsAndChecker(script, initial_stack, SCRIPT_VERIFY_NONE, checker, expected_cost)};
    Assert(exact.ok);
    ...
    if (expected_cost > 0) {
        const EvalOutcome low{EvalTapscriptV2WithFlagsAndChecker(script, initial_stack, SCRIPT_VERIFY_NONE, checker, expected_cost - 1)};
        Assert(!low.ok);
        Assert(low.error == SCRIPT_ERR_VAROP_COUNT);
    }
}

inline void CheckEvalErrorBudget(const CScript& script, const Stack& initial_stack, uint64_t budget, ScriptError expected_error, uint64_t expected_remaining)
{
    const BaseSignatureChecker checker;
    const EvalOutcome outcome{EvalTapscriptV2WithFlagsAndChecker(script, initial_stack, SCRIPT_VERIFY_NONE, checker, budget)};
    Assert(!outcome.ok);
...
inline bool VerifySpend(const LeafSpend& spend, script_verify_flags flags, const BaseSignatureChecker& checker, varops::Budget& budget, ScriptError& error)
{
...
inline VerifyOutcome VerifySpend(const LeafSpend& spend, script_verify_flags flags, const BaseSignatureChecker& checker, uint64_t budget)
{
