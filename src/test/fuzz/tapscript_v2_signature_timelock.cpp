...
void CheckCodeseparator(FuzzedDataProvider& provider)
{
    ...
    for (size_t i{0}; i < segments; ++i) {
        ...
        case 1:
            append(OP_0);
            append(OP_IF);
            append(OP_CODESEPARATOR, /*executed=*/false);
            append(OP_ENDIF);
            break;
        case 2:
            append(OP_1);
    ...
    RecordingChecker low_checker;
    const EvalOutcome low{
        EvalTapscriptV2WithFlagsAndChecker(script, stack, SCRIPT_VERIFY_NONE, low_checker, exact_cost - 1)};
    Assert(!low.ok);
    Assert(low.error == SCRIPT_ERR_VAROP_COUNT);
    Assert(low_checker.schnorr_calls == 0);
}
