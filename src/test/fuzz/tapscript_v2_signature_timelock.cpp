...
void CheckSignatureOpcode(FuzzedDataProvider& provider)
{
    ...
    case 2: pubkey = {BIP118_PUBKEY_PREFIX}; break;
    case 3:
        pubkey.assign(33, 0x02);
        pubkey.front() = BIP118_PUBKEY_PREFIX;
        break;
    ...
    const bool anyprevout{upgradable && (pubkey.size() == 1 || pubkey.size() == 33) && pubkey.front() == BIP118_PUBKEY_PREFIX};

    RecordingChecker checker;
    const EvalOutcome exact{EvalTapscriptV2WithFlagsAndChecker(OneOp(opcode), initial, SCRIPT_VERIFY_NONE, checker, cost)};
    if (pubkey.empty()) {
        Assert(!exact.ok);
        Assert(exact.error == SCRIPT_ERR_PUBKEYTYPE);
    ...
    if (checker.schnorr_calls == 1) {
        ...
        Assert(checker.last_keyversion == KeyVersion::TAPROOT);
        Assert(checker.last_codeseparator_pos == 0xFFFFFFFFUL);
    }

    if (cost > 0) {
        RecordingChecker low_checker;
        const EvalOutcome low{EvalTapscriptV2WithFlagsAndChecker(OneOp(opcode), initial, SCRIPT_VERIFY_NONE, low_checker, cost - 1)};
        Assert(!low.ok);
    ...
    if (anyprevout) {
        RecordingChecker anyprevout_checker;
        const EvalOutcome with_flags{EvalTapscriptV2WithFlagsAndChecker(
            OneOp(opcode), initial, SCRIPT_VERIFY_ANYPREVOUT | SCRIPT_VERIFY_DISCOURAGE_ANYPREVOUT, anyprevout_checker, cost)};
        Assert(with_flags.ok == exact.ok);
        Assert(with_flags.error == exact.error);
        Assert(with_flags.remaining_budget == exact.remaining_budget);
        Assert(with_flags.stack == exact.stack);
        Assert(anyprevout_checker.schnorr_calls == 0);
    }
...
void CheckCodeseparator(FuzzedDataProvider& provider)
{
    ...
    RecordingChecker checker;
    const EvalOutcome exact{EvalTapscriptV2WithFlagsAndChecker(script, stack, SCRIPT_VERIFY_NONE, checker, COST_PER_SIGOP)};
    Assert(exact.ok);
    Assert(exact.error == SCRIPT_ERR_OK);
    Assert(exact.remaining_budget == 0);
    ...
    RecordingChecker low_checker;
    const EvalOutcome low{EvalTapscriptV2WithFlagsAndChecker(script, stack, SCRIPT_VERIFY_NONE, low_checker, COST_PER_SIGOP - 1)};
    Assert(!low.ok);
    Assert(low.error == SCRIPT_ERR_VAROP_COUNT);
    Assert(low_checker.schnorr_calls == 0);
}
...
void CheckRepeatedSigops(FuzzedDataProvider& provider)
{
    ...
    const EvalOutcome low{EvalTapscriptV2WithFlagsAndChecker(script, stack, SCRIPT_VERIFY_NONE, low_checker, low_budget)};
    Assert(!low.ok);
    Assert(low.error == SCRIPT_ERR_VAROP_COUNT);
...
Bytes ConsumeTimelockOperand(FuzzedDataProvider& provider)
{
    ...
    if (provider.ConsumeBool()) operand.resize(provider.ConsumeIntegralInRange<size_t>(operand.size(), operand.size() + 8));
    return operand;
}
...
void CheckTimelock(FuzzedDataProvider& provider)
{
    ...
    if (disabled || checker_result) {
        ...
        if (cost > 0) {
            RecordingChecker low_checker;
            const EvalOutcome low{EvalTapscriptV2WithFlagsAndChecker(OneOp(opcode), {operand}, flags, low_checker, cost - 1)};
            Assert(!low.ok);
            Assert(low.error == SCRIPT_ERR_VAROP_COUNT);
        }
    } else {
