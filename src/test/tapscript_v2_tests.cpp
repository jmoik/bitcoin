...
static constexpr script_verify_flags TAPSCRIPT_V2_ACTIVE_INQUISITION_OPCODE_FLAGS{
    TAPSCRIPT_V2_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_CHECKSIGFROMSTACK | SCRIPT_VERIFY_INTERNALKEY};

static uint64_t FinalSuccessCost(size_t size)
{
    return varops::comparingzero_cost(size);
}
...
BOOST_AUTO_TEST_CASE(tapleaf_versions_isolate_restored_opcodes)
{
    ...
    outcome = VerifyTaprootLeafWithFlags(cat_script, {}, TAPROOT_LEAF_TAPSCRIPT,
                                         TAPROOT_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_DISCOURAGE_OP_CAT, 0);
    BOOST_CHECK(!outcome.ok);
    BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_DISCOURAGE_OP_CAT);

    outcome = VerifyTaprootLeafWithFlags(cat_script, {}, TAPROOT_LEAF_TAPSCRIPT,
                                         TAPROOT_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_OP_CAT, 0);
    BOOST_CHECK(!outcome.ok);
    BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_INVALID_STACK_OPERATION);
}
...
BOOST_AUTO_TEST_CASE(splice_opcodes_follow_bip441_byte_ranges)
{
    ...
    CheckEval(substr, {Bytes("abcdef"), Num(2), Num(3)}, {Bytes("cde")},
              varops::lengthconv_cost(1) + varops::lengthconv_cost(1) + 3 * varops::COST_COPYING);
    CheckError(substr, {Bytes("abcdef"), Num(2), Num(3)},
               varops::lengthconv_cost(1) + varops::lengthconv_cost(1) + 3 * varops::COST_COPYING - 1,
               SCRIPT_ERR_VAROP_COUNT);
    CheckEval(substr, {Bytes("abcdef"), Num(9), Num(3)}, {Bytes({})},
              varops::lengthconv_cost(1) + varops::lengthconv_cost(1));
    CheckEval(substr, {Bytes("abcdef"), Num(6), Num(3)}, {Bytes({})},
              varops::lengthconv_cost(1) + varops::lengthconv_cost(1));
    CheckEval(substr, {Bytes("abcdef"), Num(4), Num(9)}, {Bytes("ef")},
              varops::lengthconv_cost(1) + varops::lengthconv_cost(1) + 2 * varops::COST_COPYING);
    CheckEval(substr, {Bytes("abcdef"), Num(2), LargerThanU64()}, {Bytes("cdef")},
              varops::lengthconv_cost(1) + varops::lengthconv_cost(9) + 4 * varops::COST_COPYING);
    CheckEval(substr, {Bytes("abcdef"), LargerThanU64(), LargerThanU64()}, {Bytes({})},
              varops::lengthconv_cost(9) + varops::lengthconv_cost(9));

    CheckEval(OneOp(OP_LEFT), {Bytes("abcdef"), Num(0)}, {Bytes({})}, 0);
    CheckEval(OneOp(OP_LEFT), {Bytes("abcdef"), Num(2)}, {Bytes("ab")}, varops::lengthconv_cost(1));
    CheckError(OneOp(OP_LEFT), {Bytes("abcdef"), Num(2)}, varops::lengthconv_cost(1) - 1,
               SCRIPT_ERR_VAROP_COUNT);
    CheckEval(OneOp(OP_LEFT), {Bytes("abcdef"), Num(9)}, {Bytes("abcdef")}, varops::lengthconv_cost(1));
    CheckEval(OneOp(OP_LEFT), {Bytes("abcdef"), LargerThanU64()}, {Bytes("abcdef")}, varops::lengthconv_cost(9));
    CheckEval(OneOp(OP_LEFT), {Bytes("abcdef"), Bytes({0x02, 0x00, 0x00})}, {Bytes("ab")},
              varops::lengthconv_cost(3));

    CheckEval(OneOp(OP_RIGHT), {Bytes("abcdef"), Num(0)}, {Bytes({})}, 0);
    CheckEval(OneOp(OP_RIGHT), {Bytes("abcdef"), Num(2)}, {Bytes("ef")},
              varops::lengthconv_cost(1) + 2 * varops::COST_COPYING);
    CheckError(OneOp(OP_RIGHT), {Bytes("abcdef"), Num(2)},
               varops::lengthconv_cost(1) + 2 * varops::COST_COPYING - 1,
               SCRIPT_ERR_VAROP_COUNT);
    CheckEval(OneOp(OP_RIGHT), {Bytes("abcdef"), Num(9)}, {Bytes("abcdef")},
              varops::lengthconv_cost(1) + 6 * varops::COST_COPYING);
    CheckEval(OneOp(OP_RIGHT), {Bytes("abcdef"), LargerThanU64()}, {Bytes("abcdef")},
              varops::lengthconv_cost(9) + 6 * varops::COST_COPYING);
    CheckEval(OneOp(OP_RIGHT), {Bytes("abcdef"), Bytes({0x02, 0x00, 0x00})}, {Bytes("ef")},
              varops::lengthconv_cost(3) + 2 * varops::COST_COPYING);
}
...
BOOST_AUTO_TEST_CASE(length_operands_charge_encoded_width_even_when_value_is_zero)
{
    ...
    const uint64_t padded_zero_cost{varops::lengthconv_cost(padded_zero.size())};
...
{
    CheckEval(OneOp(OP_INVERT), {Bytes({0x00, 0xff})}, {Bytes({0xff, 0x00})}, varops::invert_cost(2));
    CheckEval(OneOp(OP_AND), {Bytes({0xff, 0xff}), Bytes({0x0f})}, {Bytes({0x0f, 0x00})}, varops::and_cost(2, 1));
    CheckEval(OneOp(OP_OR), {Bytes({0x00, 0x00}), Bytes({0x00})}, {Bytes({0x00, 0x00})}, varops::or_cost(2, 1));
    CheckEval(OneOp(OP_XOR), {Bytes({0x01, 0x00}), Bytes({0x01})}, {Bytes({0x00, 0x00})}, varops::xor_cost(2, 1));
}
...
BOOST_AUTO_TEST_CASE(restored_shift_opcodes_are_raw_bitshifts)
{
    ...
    CheckEval(OneOp(OP_LSHIFT), {Bytes({0x01}), Num(1)}, {Bytes({0x02, 0x00})},
              varops::lengthconv_cost(1) + 1 * varops::COST_COPYING + varops::upshift_bitshift_cost(1, 0));
    CheckError(OneOp(OP_LSHIFT), {Bytes({0x01}), Num(1)},
               varops::lengthconv_cost(1) + 1 * varops::COST_COPYING + varops::upshift_bitshift_cost(1, 0) - 1,
               SCRIPT_ERR_VAROP_COUNT);
    CheckEval(OneOp(OP_LSHIFT), {Bytes({}), Num(1)}, {Bytes({0x00})}, varops::lengthconv_cost(1));
    CheckEval(OneOp(OP_LSHIFT), {Bytes({0x01}), Num(8)}, {Bytes({0x00, 0x01})},
              varops::lengthconv_cost(1) + 1 * varops::COST_FAST + 1 * varops::COST_COPYING);
    CheckError(OneOp(OP_LSHIFT), {Bytes({0x01}), Num(8)},
               varops::lengthconv_cost(1) + 1 * varops::COST_FAST + 1 * varops::COST_COPYING - 1,
               SCRIPT_ERR_VAROP_COUNT);
    ...
    CheckError(OneOp(OP_RSHIFT), {Bytes({0x02, 0x00}), Num(1)},
               varops::lengthconv_cost(1) + 2 * varops::COST_COPYING - 1,
               SCRIPT_ERR_VAROP_COUNT);
    CheckEval(OneOp(OP_RSHIFT), {Bytes({}), Num(1)}, {Bytes({})}, varops::lengthconv_cost(1));
    CheckEval(OneOp(OP_RSHIFT), {Bytes({0xff}), Num(8)}, {Bytes({})}, varops::lengthconv_cost(1));
    CheckEval(OneOp(OP_RSHIFT), {Bytes({0x12, 0x34}), LargerThanU64()}, {Bytes({})},
              varops::lengthconv_cost(9));
...
{
    CheckEval(OneOp(OP_2MUL), {Bytes({0x80})}, {Bytes({0x00, 0x01})}, varops::twomul_cost(1));
    CheckEval(OneOp(OP_2DIV), {Bytes({0x01})}, {Bytes({})}, varops::twodiv_cost(1));

    CheckEval(OneOp(OP_MUL), {Bytes({0xff, 0xff}), Bytes({0x02})}, {Bytes({0xfe, 0xff, 0x01})}, varops::mul_cost(2, 1));
    CheckEval(OneOp(OP_DIV), {Bytes({0x39, 0x30}), Bytes({0x64})}, {Bytes({0x7b})}, varops::div_cost(2, 1));
    CheckEval(OneOp(OP_MOD), {Bytes({0x39, 0x30}), Bytes({0x64})}, {Bytes({0x2d})}, varops::mod_cost(2, 1));

    for (const valtype& divisor : {Bytes({}), Bytes({0x00, 0x00})}) {
        CheckError(OneOp(OP_DIV), {Bytes({0x01}), divisor}, varops::div_cost(1, divisor.size()), SCRIPT_ERR_DIVIDE_BY_ZERO);
        CheckError(OneOp(OP_MOD), {Bytes({0x01}), divisor}, varops::mod_cost(1, divisor.size()), SCRIPT_ERR_DIVIDE_BY_ZERO);
    }
}

BOOST_AUTO_TEST_CASE(extended_arithmetic_is_unsigned_and_normalized)
{
    CheckEval(OneOp(OP_1ADD), {Bytes({0xff})}, {Bytes({0x00, 0x01})}, varops::add_cost(1, 1));
    CheckEval(OneOp(OP_1SUB), {Bytes({0x01})}, {Bytes({})}, varops::sub_cost(1, 1));
    CheckEval(OneOp(OP_ADD), {Bytes({0x01, 0x00, 0x00}), Bytes({})}, {Bytes({0x01})}, varops::add_cost(3, 0));
    CheckEval(OneOp(OP_SUB), {Bytes({0x00, 0x01}), Bytes({0x01})}, {Bytes({0xff})}, varops::sub_cost(2, 1));

    CheckError(OneOp(OP_1SUB), {Bytes({})}, varops::sub_cost(0, 1), SCRIPT_ERR_SUB_UNDERFLOW);
    CheckError(OneOp(OP_1SUB), {Bytes({0x00})}, varops::sub_cost(1, 1), SCRIPT_ERR_SUB_UNDERFLOW);
    CheckError(OneOp(OP_SUB), {Bytes({0x01}), Bytes({0x02})}, varops::sub_cost(1, 1), SCRIPT_ERR_SUB_UNDERFLOW);
    CheckError(OneOp(OP_SUB), {Bytes({0x00}), Bytes({0x01})}, varops::sub_cost(1, 1), SCRIPT_ERR_SUB_UNDERFLOW);

    CheckEval(OneOp(OP_MIN), {Bytes({0x01, 0x00}), Bytes({0x02})}, {Bytes({0x01})}, varops::minmax_cost(2, 1));
    CheckEval(OneOp(OP_MAX), {Bytes({0x01, 0x00}), Bytes({0x02})}, {Bytes({0x02})}, varops::minmax_cost(2, 1));
}
...
BOOST_AUTO_TEST_CASE(checksigadd_failure_normalizes_numeric_operand)
{
    ...
    const uint64_t cost{varops::checksigadd_incr_cost(nonminimal_one.size()) +
...
BOOST_AUTO_TEST_CASE(checksigadd_enforces_numeric_result_boundaries)
{
...

    {
        ...
        const uint64_t cost{varops::COST_PER_SIGOP + varops::checksigadd_incr_cost(all_ff.size())};
    ...
    {
        RecordingChecker checker;
        const uint64_t cost{varops::COST_PER_SIGOP + varops::checksigadd_incr_cost(all_zero.size())};
    ...
    {
        RecordingChecker checker;
        const uint64_t cost{varops::checksigadd_incr_cost(all_zero.size())};
...

    {
        ...
        const uint64_t cost{varops::COST_PER_SIGOP + varops::checksigadd_incr_cost(number.size())};
        const EvalOutcome outcome{EvalTapscriptV2WithFlagsAndChecker(script, {nonempty_sig, number, empty_pubkey},
                                                              SCRIPT_VERIFY_NONE, checker, cost)};
        BOOST_CHECK(!outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_PUBKEYTYPE);
        BOOST_CHECK_EQUAL(outcome.remaining_budget, 0);
        BOOST_CHECK_EQUAL(checker.schnorr_calls, 0);
    }
}
...
BOOST_AUTO_TEST_CASE(bip440_costed_legacy_ops_use_unsigned_values)
{
    ...
    CheckEval(OneOp(OP_VERIFY), {Bytes({0x00, 0x01})}, {}, varops::lengthconv_cost(2));
    CheckError(OneOp(OP_VERIFY), {Bytes({})}, 0, SCRIPT_ERR_VERIFY);
    CheckError(OneOp(OP_VERIFY), {Bytes({0x00, 0x00})}, varops::comparingzero_cost(2), SCRIPT_ERR_VERIFY);

    CheckEval(OneOp(OP_NOT), {Bytes({0x00, 0x00})}, {Bytes({0x01})}, varops::comparingzero_cost(2));
    CheckEval(OneOp(OP_0NOTEQUAL), {Bytes({0x00, 0x01})}, {Bytes({0x01})}, varops::comparingzero_cost(2));
    CheckEval(OneOp(OP_NUMEQUAL), {Bytes({0x01}), Bytes({0x01, 0x00, 0x00})}, {Bytes({0x01})}, varops::comparing_cost(1, 3));
    CheckEval(OneOp(OP_BOOLAND), {Bytes({0x01, 0x00}), Bytes({})}, {Bytes({})}, varops::booland_cost(2, 0));

    CheckEval(OneOp(OP_PICK), {Bytes("bottom"), Bytes("top"), Bytes({0x01, 0x00, 0x00})}, {Bytes("bottom"), Bytes("top"), Bytes("bottom")},
              varops::lengthconv_cost(3) + 6 * varops::COST_COPYING);
    CheckEval(OneOp(OP_ROLL), {Bytes("bottom"), Bytes("top"), Bytes({0x01, 0x00, 0x00})}, {Bytes("top"), Bytes("bottom")},
              varops::lengthconv_cost(3) + 1 * varops::COST_ROLL);
}
...
BOOST_AUTO_TEST_CASE(costed_legacy_ops_report_exact_semantic_failures)
{
    ...
    CheckError(OneOp(OP_NUMEQUALVERIFY), {Bytes({0x01}), Bytes({0x02})},
               varops::comparing_cost(1, 1), SCRIPT_ERR_NUMEQUALVERIFY);

    CheckError(OneOp(OP_PICK), {Bytes("only"), Num(1)},
               varops::lengthconv_cost(1), SCRIPT_ERR_INVALID_STACK_OPERATION);
...
BOOST_AUTO_TEST_CASE(locktime_sequence_operands_must_fit_transaction_fields)
{
...

    {
        ...
        const EvalOutcome outcome{EvalTapscriptV2WithFlagsAndChecker(cltv_script, {padded_uint32_max},
                                                      ...
                                                      checker, varops::lengthconv_cost(padded_uint32_max.size()))};
        BOOST_CHECK(outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_OK);
        BOOST_CHECK_EQUAL(outcome.remaining_budget, 0);
...

    {
        ...
        const EvalOutcome outcome{EvalTapscriptV2WithFlagsAndChecker(cltv_script, {uint32_overflow},
                                                      SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY,
                                                      checker, varops::lengthconv_cost(uint32_overflow.size()))};
        BOOST_CHECK(!outcome.ok);
...

    {
        ...
        const EvalOutcome outcome{EvalTapscriptV2WithFlagsAndChecker(csv_script, {padded_one},
                                                      ...
                                                      checker, varops::lengthconv_cost(padded_one.size()))};
        BOOST_CHECK(outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_OK);
        BOOST_CHECK_EQUAL(outcome.remaining_budget, 0);
...

    {
        ...
        const EvalOutcome outcome{EvalTapscriptV2WithFlagsAndChecker(csv_script, {overflow_with_csv_disable_flag},
                                                      SCRIPT_VERIFY_CHECKSEQUENCEVERIFY,
                                                      checker, varops::lengthconv_cost(overflow_with_csv_disable_flag.size()))};
        BOOST_CHECK(!outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_UNSATISFIED_LOCKTIME);
        BOOST_CHECK_EQUAL(checker.sequence_calls, 0);
    }
...
BOOST_AUTO_TEST_CASE(tapscript_v2_stack_limits_are_enforced)
{
    ...
    CheckError(OneOp(OP_IFDUP), max_count_true_top,
               varops::comparingzero_cost(1) + varops::COST_COPYING,
...
BOOST_AUTO_TEST_CASE(tapscript_v2_witness_initial_stack_limits_are_enforced)
{
    ...
    outcome = VerifyTapscriptV2(empty_script, {too_large_element}, 1'000);
    BOOST_CHECK(!outcome.ok);
    BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_STACK_ELEMENT_SIZE);

    outcome = VerifyTapscriptV2(empty_script, {four_mb, four_mb, Bytes({0x01})}, 1'000);
    BOOST_CHECK(!outcome.ok);
    BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_TOTAL_STACK_SIZE);
}
...
BOOST_AUTO_TEST_CASE(restored_ops_enforce_stack_element_limit_edges)
{
    ...
    CheckEval(OneOp(OP_LSHIFT), {one_byte_below_max, Num(8)}, {byte_shifted_to_max},
              varops::lengthconv_cost(1) + varops::COST_FAST +
    ...
    CheckError(OneOp(OP_1ADD), {max_ff_element}, varops::add_cost(max_ff_element.size(), 1),
               SCRIPT_ERR_STACK_ELEMENT_SIZE);
    CheckError(OneOp(OP_2MUL), {max_ff_element}, varops::twomul_cost(max_ff_element.size()),
               SCRIPT_ERR_STACK_ELEMENT_SIZE);
}
...
BOOST_AUTO_TEST_CASE(varops_budget_must_cover_exact_bip_cost)
{
    ...
    CheckEval(OneOp(OP_MUL), {Bytes({0xff}), Bytes({0xff})}, {Bytes({0x01, 0xfe})}, varops::mul_cost(1, 1));
    CheckError(OneOp(OP_MUL), {Bytes({0xff}), Bytes({0xff})}, varops::mul_cost(1, 1) - 1, SCRIPT_ERR_VAROP_COUNT);

    CheckEval(OneOp(OP_DIV), {Bytes({0x39, 0x30}), Bytes({0x64})}, {Bytes({0x7b})}, varops::div_cost(2, 1));
    CheckError(OneOp(OP_DIV), {Bytes({0x39, 0x30}), Bytes({0x64})}, varops::div_cost(2, 1) - 1, SCRIPT_ERR_VAROP_COUNT);

    CheckEval(OneOp(OP_MOD), {Bytes({0x39, 0x30}), Bytes({0x64})}, {Bytes({0x2d})}, varops::mod_cost(2, 1));
    CheckError(OneOp(OP_MOD), {Bytes({0x39, 0x30}), Bytes({0x64})}, varops::mod_cost(2, 1) - 1, SCRIPT_ERR_VAROP_COUNT);
    CheckError(OneOp(OP_MOD), {Bytes({0x01}), Bytes({})}, varops::mod_cost(1, 0) - 1, SCRIPT_ERR_VAROP_COUNT);
    ...
    const uint64_t empty_checksigadd_cost{varops::checksigadd_incr_cost(nonminimal_one.size())};
    ...
    const uint64_t nonempty_checksigadd_cost{varops::COST_PER_SIGOP + varops::checksigadd_incr_cost(nonminimal_one.size())};
...
BOOST_AUTO_TEST_CASE(signature_varops_charge_depends_on_nonempty_signature)
{
    ...
    const uint64_t checksigadd_cost{varops::checksigadd_incr_cost(zero.size())};
    ...
    const uint64_t nonempty_checksigadd_cost{varops::COST_PER_SIGOP + varops::checksigadd_incr_cost(zero.size())};
...
BOOST_AUTO_TEST_CASE(op_success_redefinitions_are_checked_before_execution)
{
    ...
    for (const opcodetype opcode : {OP_INTERNALKEY, OP_CHECKSIGFROMSTACK}) {
        CScript script;
        script << opcode << OP_RETURN;
        ScriptError error{SCRIPT_ERR_UNKNOWN_ERROR};
        const std::optional<bool> result{CheckTapscriptOpSuccess(script, TAPSCRIPT_V2_ACTIVE_INQUISITION_OPCODE_FLAGS, SigVersion::TAPSCRIPT_V2, &error)};
        BOOST_REQUIRE(result.has_value());
        BOOST_CHECK(*result);
        BOOST_CHECK_EQUAL(error, SCRIPT_ERR_OK);

        error = SCRIPT_ERR_UNKNOWN_ERROR;
        const std::optional<bool> discouraged{CheckTapscriptOpSuccess(script, TAPSCRIPT_V2_ACTIVE_INQUISITION_OPCODE_FLAGS | SCRIPT_VERIFY_DISCOURAGE_OP_SUCCESS,
                                                       SigVersion::TAPSCRIPT_V2, &error)};
        BOOST_REQUIRE(discouraged.has_value());
        BOOST_CHECK(!*discouraged);
        BOOST_CHECK_EQUAL(error, SCRIPT_ERR_DISCOURAGE_OP_SUCCESS);

        EvalOutcome outcome{VerifyTapscriptV2WithFlags(script, {}, TAPSCRIPT_V2_ACTIVE_INQUISITION_OPCODE_FLAGS, 0)};
        BOOST_CHECK(outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_OK);
    }
...
BOOST_AUTO_TEST_CASE(checktemplateverify_is_upgradable_nop_in_tapscript_v2)
{
    const valtype ctv_hash(32, 0x01);
    CScript script;
    script << ctv_hash << OP_CHECKTEMPLATEVERIFY;

    const script_verify_flags flags{TAPSCRIPT_V2_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_CHECKTEMPLATEVERIFY};
    ...
    for (const opcodetype opcode : {OP_NOP1, OP_NOP4, OP_NOP5, OP_NOP6, OP_NOP7, OP_NOP8, OP_NOP9, OP_NOP10}) {
        CScript nop_script;
        nop_script << opcode << OP_1;
        outcome = VerifyTapscriptV2WithFlags(nop_script, {}, flags, FinalSuccessCost(1));
        BOOST_CHECK(outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_OK);

        outcome = VerifyTapscriptV2WithFlags(nop_script, {}, flags | SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS,
                                             FinalSuccessCost(1));
        BOOST_CHECK(!outcome.ok);
        BOOST_CHECK_EQUAL(outcome.error, SCRIPT_ERR_DISCOURAGE_UPGRADABLE_NOPS);
    }
...
BOOST_AUTO_TEST_CASE(tapscript_v2_treats_anyprevout_pubkeys_as_unknown_pubkey_type)
{
    valtype anyprevout_xonly_pubkey(33, 0x02);
    anyprevout_xonly_pubkey.front() = BIP118_PUBKEY_PREFIX;
    const valtype invalid_signature(64, 0x01);
    const script_verify_flags flags{
        TAPSCRIPT_V2_SCRIPT_VERIFY_FLAGS |
        SCRIPT_VERIFY_ANYPREVOUT |
        SCRIPT_VERIFY_DISCOURAGE_ANYPREVOUT};
    const uint64_t cost{varops::COST_PER_SIGOP + FinalSuccessCost(1)};

    for (const valtype& pubkey : {Bytes({BIP118_PUBKEY_PREFIX}), anyprevout_xonly_pubkey}) {
        CScript script;
...
BOOST_AUTO_TEST_CASE(unexecuted_branches_do_not_charge_or_execute_costed_ops)
{
    ...
    CScript skipped_else;
    skipped_else << OP_1 << OP_IF << OP_2 << OP_ELSE << OP_CAT << OP_MUL << OP_SHA256 << OP_ENDIF;
    CheckEval(skipped_else, {}, {Bytes({0x02})}, 0);
}

BOOST_AUTO_TEST_CASE(tapscript_v2_standard_verify_uses_explicit_nonconsensus_budget)
{
    ...
    const bool ok{VerifyScript(CScript{}, script_pub_key, &witness, STANDARD_SCRIPT_VERIFY_FLAGS, BaseSignatureChecker{}, &error, varops_budget)};
    BOOST_CHECK(ok);
    BOOST_CHECK_EQUAL(error, SCRIPT_ERR_OK);
}
...
BOOST_AUTO_TEST_CASE(tapscript_v2_psbt_finalized_witness_is_signed_and_verified)
{
    ...
    PartiallySignedTransaction psbt{spend.tx};
...
BOOST_AUTO_TEST_CASE(tapscript_v2_psbt_and_signtransaction_use_transaction_wide_varops_budget)
{
    ...
    const uint64_t per_input_cost{varops::mul_cost(operand_size, operand_size)};
    ...
    BOOST_CHECK(!SignTransaction(tx, &DUMMY_SIGNING_PROVIDER, coins, SIGHASH_DEFAULT, input_errors));
...
BOOST_AUTO_TEST_CASE(tapscript_v2_psbt_single_input_over_finalized_budget_is_rejected)
{
    ...
    const uint64_t input_cost{varops::mul_cost(operand_size, operand_size)};
...
BOOST_AUTO_TEST_CASE(tapscript_v2_signtransaction_accepts_finalized_witness)
{
    ...
    std::map<int, bilingual_str> input_errors;
    BOOST_CHECK(SignTransaction(spend.tx, &DUMMY_SIGNING_PROVIDER, coins, SIGHASH_DEFAULT, input_errors));
