...
class TapScriptV2Test(TaprootTest):
    ...
    def test_transaction_wide_varops_budget(self):
        ...
        def make_budget_spender(name, script, version=LEAF_VERSION_TAPSCRIPT_V2, inputs=None):
            ...
            return make_spender(name, tap=tap, leaf=name, inputs=inputs if inputs is not None else [padding, operand, operand])
        ...
        # An otherwise valid v1 input cannot fund the v2 input's multiplication.
        mixed_tx = make_spend_tx(funded[4:])
        excluded_weight = 4 * len(mixed_tx.vin[1].serialize()) + len(mixed_tx.wit.vtxinwit[1].serialize())
        assert expensive_input_cost < mixed_tx.get_weight() * VAROPS_BUDGET_PER_WEIGHT
        assert expensive_input_cost > (mixed_tx.get_weight() - excluded_weight) * VAROPS_BUDGET_PER_WEIGHT
        result = node.testmempoolaccept([mixed_tx.serialize().hex()], maxfeerate=0)[0]
        assert not result["allowed"], result
        self.block_submit(
            node, [mixed_tx], "v1 input weight does not fund v2 execution",
            witness=True, accept=False, cb_pubkey=host_pubkey, fees=50_000,
            sigops_weight=MAX_BLOCK_SIGOPS_WEIGHT, err_msg=ERR_VAROP_COUNT["err_msg"],
        )
