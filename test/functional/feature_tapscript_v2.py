...
from test_framework.key import compute_xonly_pubkey, generate_privkey, sign_schnorr, tweak_add_privkey
...
ERR_DIVIDE_BY_ZERO = {"err_msg": "OP_DIV or OP_MOD by zero"}
ERR_LOCKTIME = {"err_msg": "Locktime requirement not satisfied"}
ERR_SUB_UNDERFLOW = {"err_msg": "OP_SUB or OP_1SUB result would be negative"}
...
ERR_HASH_OPERAND_SIZE = {"err_msg": "OP_RIPEMD160 or OP_SHA1 operand exceeds 520 bytes"}
...
def tapscript_v2_spenders():
    ...
    # OP_1NEGATE, OP_NEGATE, and OP_ABS are OP_SUCCESSx in v2 and bypass
    # execution of later failing opcodes.
    for name, success_op in (("1negate", OP_1NEGATE), ("negate", OP_NEGATE), ("abs", OP_ABS)):
        tap = taproot_construct(pubs[0], [
            (f"{name}_success", CScript([success_op, OP_RETURN]), LEAF_VERSION_TAPSCRIPT_V2),
            (f"{name}_return", CScript([OP_RETURN]), LEAF_VERSION_TAPSCRIPT_V2),
        ])
        add_spender(
            spenders,
            f"v2/opsuccess_{name}",
            tap=tap,
            leaf=f"{name}_success",
            failure={"leaf": f"{name}_return"},
            standard=False,
            **ERR_OP_RETURN,
        )
    ...
    # OP_DEPTH and OP_SIZE write minimal unsigned values in v2.
    depth_count = 130
    ...
    size_script = CScript([OP_SIZE, v2_num(256), OP_EQUALVERIFY, OP_DROP, OP_1])
    tap = taproot_construct(pubs[0], [("size", size_script, LEAF_VERSION_TAPSCRIPT_V2)])
    add_spender(
        spenders,
        "v2/size_unsigned_minimal",
        tap=tap,
        leaf="size",
        inputs=[b"b" * 256],
        failure={"inputs": [b"b" * 255]},
        **ERR_EQUALVERIFY,
    )

    # Splice opcodes restored under 0xc2.
    cat_a = b"x" * 400
    cat_b = b"y" * 180
    cat_script = CScript([OP_CAT, cat_a + cat_b, OP_EQUAL])
    tap = taproot_construct(pubs[0], [
        ("cat_v2", cat_script, LEAF_VERSION_TAPSCRIPT_V2),
        ("cat_c0", cat_script, LEAF_VERSION_TAPSCRIPT),
    ])
    add_spender(
        spenders,
    ...
    add_spender_for_script(
        ...
        pubs[0],
        "v2/substr",
        CScript([OP_SUBSTR, b"cde", OP_EQUAL]),
        inputs=[b"abcdef", v2_num(2), v2_num(3)],
        failure_inputs=[b"abcdef", v2_num(1), v2_num(3)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/substr_clamps_past_end",
        CScript([OP_SUBSTR, b"", OP_EQUAL]),
        inputs=[b"abcdef", v2_num(99), v2_num(4)],
        failure_inputs=[b"abcdef", v2_num(5), v2_num(1)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/left",
        CScript([OP_LEFT, b"abc", OP_EQUAL]),
        inputs=[b"abcdef", v2_num(3)],
        failure_inputs=[b"abcdef", v2_num(4)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/left_clamps_past_end",
        CScript([OP_LEFT, b"abcdef", OP_EQUAL]),
        inputs=[b"abcdef", v2_num(99)],
        failure_inputs=[b"abcdef", v2_num(5)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/right",
        CScript([OP_RIGHT, b"ef", OP_EQUAL]),
        inputs=[b"abcdef", v2_num(2)],
        failure_inputs=[b"abcdef", v2_num(3)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/right_clamps_past_end",
        CScript([OP_RIGHT, b"abcdef", OP_EQUAL]),
        inputs=[b"abcdef", v2_num(99)],
        failure_inputs=[b"abcdef", v2_num(5)],
    )

    # Bitwise opcodes operate on little-endian byte strings and do not
    # normalize trailing zeroes.
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/invert",
        CScript([OP_INVERT, b"\xff\x00", OP_EQUAL]),
        inputs=[b"\x00\xff"],
        failure_inputs=[b"\x00\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/and",
        CScript([OP_AND, b"\x0f\x00", OP_EQUAL]),
        inputs=[b"\xff\xff", b"\x0f"],
        failure_inputs=[b"\xff\xff", b"\xf0"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/or",
        CScript([OP_OR, b"\x0f\x01", OP_EQUAL]),
        inputs=[b"\x0f\x00", b"\x00\x01"],
        failure_inputs=[b"\x0f\x00", b"\x00\x02"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/xor",
        CScript([OP_XOR, b"\x0e\x01", OP_EQUAL]),
        inputs=[b"\x0f\x00", b"\x01\x01"],
        failure_inputs=[b"\x0f\x00", b"\x01\x02"],
    )

    # Shifts expose byte order and preserve/trim according to the v2 rules.
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/lshift",
        CScript([OP_LSHIFT, b"\x00\x01", OP_EQUAL]),
        inputs=[b"\x80", v2_num(1)],
        failure_inputs=[b"\x80", v2_num(2)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/rshift",
        CScript([OP_RSHIFT, OP_1, OP_EQUAL]),
        inputs=[b"\x00\x01", v2_num(8)],
        failure_inputs=[b"\x00\x01", v2_num(7)],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
    ...
    # Arithmetic uses unsigned arbitrary-length values and normalizes results.
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/1add",
        CScript([OP_1ADD, b"\x00\x01", OP_EQUAL]),
        inputs=[b"\xff"],
        failure_inputs=[b"\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/1sub",
        CScript([OP_1SUB, OP_0, OP_EQUAL]),
        inputs=[b"\x01"],
        failure_inputs=[b""],
        failure_err=ERR_SUB_UNDERFLOW,
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/2mul",
        CScript([OP_2MUL, b"\x00\x01", OP_EQUAL]),
        inputs=[b"\x80"],
        failure_inputs=[b"\x40"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/2div",
        CScript([OP_2DIV, b"\x80", OP_EQUAL]),
        inputs=[b"\x00\x01"],
        failure_inputs=[b"\x01"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/add_normalizes",
        CScript([OP_ADD, OP_1, OP_EQUAL]),
        inputs=[b"\x01\x00\x00", b""],
        failure_inputs=[b"\x01\x00\x00", b"\x01"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/sub_unsigned",
        CScript([OP_SUB, b"\xff", OP_EQUAL]),
        inputs=[b"\x00\x01", b"\x01"],
        failure_inputs=[b"\x01", b"\x02"],
        failure_err=ERR_SUB_UNDERFLOW,
    )
    add_spender_for_script(
        spenders,
    ...
    add_spender_for_script(
        ...
        pubs[0],
        "v2/div",
        CScript([OP_DIV, b"\x7b", OP_EQUAL]),
        inputs=[v2_num(12345), v2_num(100)],
        failure_inputs=[v2_num(12345), b""],
        failure_err=ERR_DIVIDE_BY_ZERO,
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/mod",
        CScript([OP_MOD, b"\x2d", OP_EQUAL]),
        inputs=[v2_num(12345), v2_num(100)],
        failure_inputs=[v2_num(12345), b"\x00\x00"],
        failure_err=ERR_DIVIDE_BY_ZERO,
    )

    # Comparison/control opcodes read arbitrary-length unsigned values.
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/verify_wide_true",
        CScript([OP_VERIFY, OP_1]),
        inputs=[b"\x00" * 9 + b"\x01"],
        failure_inputs=[b"\x00" * 10],
        failure_err={"err_msg": "Script failed an OP_VERIFY operation"},
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        ...
        CScript([OP_NOT]),
        inputs=[b"\x00" * 10],
        failure_inputs=[b"\x00" * 9 + b"\x01"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/numequal_ignores_trailing_zeroes",
        CScript([OP_NUMEQUAL]),
        inputs=[b"\x01", b"\x01\x00\x00\x00\x00"],
        failure_inputs=[b"\x01", b"\x02\x00\x00\x00\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/booland_wide",
        CScript([OP_BOOLAND, OP_NOT]),
        inputs=[b"\x01\x00", b""],
        failure_inputs=[b"\x01\x00", b"\x02"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/within",
        CScript([OP_WITHIN]),
        inputs=[b"\x05", b"\x02", b"\x08"],
        failure_inputs=[b"\x08", b"\x02", b"\x08"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        ...
        failure_err=ERR_MINIMALIF,
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/0notequal_wide",
        CScript([OP_0NOTEQUAL]),
        inputs=[b"\x00" * 8 + b"\x01"],
        failure_inputs=[b"\x00" * 9],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        ...
        CScript([OP_BOOLOR]),
        inputs=[b"", b"\x00\x01"],
        failure_inputs=[b"", b"\x00\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/numnotequal_trailing_zeroes",
        CScript([OP_NUMNOTEQUAL, OP_NOT]),
        inputs=[b"\x01", b"\x01\x00\x00"],
        failure_inputs=[b"\x01", b"\x02\x00\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/lessthan_unsigned",
        CScript([OP_LESSTHAN]),
        inputs=[b"\xff", b"\x00\x01"],
        failure_inputs=[b"\x00\x01", b"\xff"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/greaterthan_unsigned",
        CScript([OP_GREATERTHAN]),
        inputs=[b"\x00\x01", b"\xff"],
        failure_inputs=[b"\xff", b"\x00\x01"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/lessthanorequal_trailing_zeroes",
        CScript([OP_LESSTHANOREQUAL]),
        inputs=[b"\x01", b"\x01\x00"],
        failure_inputs=[b"\x02", b"\x01\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/greaterthanorequal_trailing_zeroes",
        CScript([OP_GREATERTHANOREQUAL]),
        inputs=[b"\x01\x00", b"\x01"],
        failure_inputs=[b"\x01", b"\x02\x00"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/min_normalizes",
        CScript([OP_MIN, OP_1, OP_EQUAL]),
        inputs=[b"\x01\x00", b"\x02"],
        failure_inputs=[b"\x02", b"\x03"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/max_normalizes",
        CScript([OP_MAX, b"\x02", OP_EQUAL]),
        inputs=[b"\x01\x00", b"\x02"],
        failure_inputs=[b"\x01", b"\x01\x00"],
    )

    # Stack manipulation has v2 copying/length conversion semantics.
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/ifdup",
        CScript([OP_IFDUP, OP_2DUP, OP_EQUALVERIFY, OP_DROP, OP_DROP, OP_1]),
        inputs=[b"\x01\x00"],
        failure_inputs=[b""],
        failure_err=ERR_STACK_EMPTY,
    )
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/tuck",
        CScript([OP_TUCK, OP_CAT, OP_CAT, b"bab", OP_EQUAL]),
        inputs=[b"a", b"b"],
        failure_inputs=[b"a", b"c"],
    )
    add_spender_for_script(
        spenders,
        pubs[0],
    ...
    add_spender_for_script(
        ...
        pubs[0],
        "v2/roll_wide_index",
        CScript([OP_ROLL, b"a", OP_EQUALVERIFY, OP_DROP, OP_1]),
        inputs=[b"a", b"b", b"\x01\x00\x00\x00\x00"],
        failure_inputs=[b"a", b"b", b"\x02\x00\x00\x00\x00"],
        failure_err=ERR_STACK_EMPTY,
    )

    # HASH256/SHA256/HASH160 are no longer capped at 520 bytes, but SHA1 and
    # RIPEMD160 remain capped in v2.
    ...
    add_spender_for_script(
        ...
        pubs[0],
        "v2/hash256_over_520",
        CScript([OP_HASH256, hash256(hash_data), OP_EQUAL]),
        inputs=[hash_data],
        failure_inputs=[hash_data[:-1]],
    )
    add_spender_for_script(
        ...
        pubs[0],
        "v2/hash160_over_520",
        CScript([OP_HASH160, hash160(hash_data), OP_EQUAL]),
        inputs=[hash_data],
        failure_inputs=[hash_data[:-1]],
    )
    add_spender_for_script(
    ...
    add_spender_for_script(
        spenders,
        pubs[0],
        "v2/ripemd160_520_limit",
        CScript([OP_RIPEMD160, ripemd160(b"r" * MAX_SCRIPT_ELEMENT_SIZE), OP_EQUAL]),
        inputs=[b"r" * MAX_SCRIPT_ELEMENT_SIZE],
        failure_inputs=[b"r" * (MAX_SCRIPT_ELEMENT_SIZE + 1)],
        failure_err=ERR_HASH_OPERAND_SIZE,
    )
    ...
    empty_checksigadd_script = CScript([pubs[2], OP_CHECKSIGADD, b"\x01", OP_EQUAL])
    ...
    # Element-size failure can also be reached by execution, without placing a
    # >4 MB item in the witness.
    oversize_shift = v2_num(MAX_TAPSCRIPT_V2_STACK_ELEMENT_SIZE * 8)
    tap = taproot_construct(pubs[0], [
        ("element_ok", drop_all_then_true(3), LEAF_VERSION_TAPSCRIPT_V2),
        ("element_fail", CScript([OP_LSHIFT]), LEAF_VERSION_TAPSCRIPT_V2),
    ])
    add_spender(
        spenders,
        "v2/stack_element_size_limit",
        tap=tap,
        leaf="element_ok",
        inputs=[b"pad" * 2600, b"\x01", oversize_shift],
        failure={"leaf": "element_fail"},
        **ERR_STACK_ELEMENT_SIZE,
    )
...
def add_spender_for_script(spenders, internal_pubkey, comment, script, *, inputs, failure_inputs, failure_err=ERR_EVAL_FALSE):
...
class TapScriptV2Test(TaprootTest):
    ...
    def run_test(self):
        ...
        self.log.info("Tapscript v2 transaction tests")
        self.test_tapscript_v2_scenarios()
    ...
    def test_tapscript_v2_leaf_before_activation(self):
        ...
        node = self.nodes[0]
        host_addr = node.getnewaddress(address_type="bech32")
        host_pubkey = bytes.fromhex(node.getaddressinfo(host_addr)["pubkey"])
    ...
    def script_restoration_state(self):
        return self.script_restoration_info()["heretical"]
    ...
    def advance_script_restoration_to_started(self):
        ...
        while self.script_restoration_state()["status"] != "started":
            ...
            period = self.script_restoration_state()["period"]
            blocks_to_next_period = period - (node.getblockcount() % period)
            if blocks_to_next_period == 0:
                blocks_to_next_period = period
            self.generate(node, blocks_to_next_period)

    def signal_script_restoration_activation(self):
        node = self.nodes[0]
        script_restoration_dep = self.script_restoration_state()
    ...
    def activate_script_restoration(self):
        ...
        self.generate(node, 2 * self.script_restoration_state()["period"])
    ...
    def fund_spenders(self, spenders, amount=10_000_000):
        node = self.nodes[0]
        host_addr = node.getnewaddress(address_type="bech32")
        host_info = node.getaddressinfo(host_addr)
        host_spk = bytes.fromhex(host_info["scriptPubKey"])
        host_pubkey = bytes.fromhex(host_info["pubkey"])
        ...
        fund_tx = self.nodes[0].signrawtransactionwithwallet(fund_tx.serialize().hex())
        fund_tx = tx_from_hex(fund_tx["hex"])
        fund_tx.rehash()

        self.init_blockinfo(node)
        self.block_submit(node, [fund_tx], "Tapscript v2 funding tx", None, host_pubkey, 10_000, MAX_BLOCK_SIGOPS_WEIGHT, True, True)

        return [
            (COutPoint(fund_tx.sha256, i), fund_tx.vout[i], spender)
    ...
    def fund_tapscript_v2(self, script, *, internal_key=None, amount=10_000_000, leaves=None, leaf_name="script"):
        ...
        funding_txid = node.sendtoaddress(address, amount / COIN)
        self.generate(node, 1)

        funding_info = node.gettransaction(funding_txid)
        funding_tx = node.decoderawtransaction(funding_info["hex"])
        expected_spk = bytes(tap.scriptPubKey).hex()
        for vout in funding_tx["vout"]:
            if vout["scriptPubKey"]["hex"] == expected_spk:
                return {
        ...
        raise AssertionError("Could not find Tapscript v2 funding output")
    ...
    def spending_tx(self, utxo, *, witness_elements=(), fee=10_000, nlocktime=0, nsequence=SEQUENCE_FINAL):
        node = self.nodes[0]
        output_addr = node.getnewaddress(address_type="bech32")
        output_spk = bytes.fromhex(node.getaddressinfo(output_addr)["scriptPubKey"])
    ...
    def script_signature(self, utxo, spending_tx, private_key, *, hashtype=SIGHASH_DEFAULT, input_index=0, key_version=None):
        spent_output = CTxOut(utxo["amount"], utxo["tap"].scriptPubKey)
        if key_version is None:
            key_version = KEY_VERSION_ANYPREVOUT if hashtype & SIGHASH_ANYPREVOUT else KEY_VERSION_TAPROOT
        sighash = TaprootSignatureHash(
            ...
            input_index,
            scriptpath=True,
            leaf_script=utxo["script"],
            codeseparator_pos=-1,
            leaf_ver=LEAF_VERSION_TAPSCRIPT_V2,
            key_ver=key_version,
        )
        signature = sign_schnorr(private_key, sighash)
        if hashtype != SIGHASH_DEFAULT:
            signature += bytes([hashtype])
        return signature
    ...
    def test_tapscript_v2_scenarios(self):
        node = self.nodes[0]

        utxo = self.fund_tapscript_v2(CScript([OP_1]))
        self.submit_and_mine(self.spending_tx(utxo), "OP_1 leaf")

        key = generate_privkey()
        pubkey = compute_xonly_pubkey(key)[0]
        checksig_script = CScript([pubkey, OP_CHECKSIG])
        utxo = self.fund_tapscript_v2(checksig_script)
        spending_tx = self.spending_tx(utxo)
        signature = self.script_signature(utxo, spending_tx, key)
        spending_tx.wit.vtxinwit[0].scriptWitness.stack.insert(0, signature)
        self.submit_and_mine(spending_tx, "OP_CHECKSIG leaf")

        # In 0xc2, 0x01-prefixed pubkeys are still upgradable pubkey types, not
        # APO keys. A well-formed APO-shaped signature is therefore policy-
        # discouraged, but consensus-valid through unknown-pubkey semantics.
        key = generate_privkey()
        apo_prefixed_pubkey = bytes([KEY_VERSION_ANYPREVOUT]) + compute_xonly_pubkey(key)[0]
        apo_prefixed_script = CScript([apo_prefixed_pubkey, OP_CHECKSIG])
        utxo = self.fund_tapscript_v2(apo_prefixed_script)
        spending_tx = self.spending_tx(utxo)
        signature = self.script_signature(
            utxo,
            spending_tx,
            key,
            hashtype=SIGHASH_ANYPREVOUT | SIGHASH_ALL,
            key_version=KEY_VERSION_ANYPREVOUT,
        )
        spending_tx.wit.vtxinwit[0].scriptWitness.stack.insert(0, signature)
        spending_tx.rehash()
        result = node.testmempoolaccept([spending_tx.serialize().hex()], maxfeerate=0)[0]
        ...
        self.submit_spend_block(
            spending_tx,
            "v2 APO-prefixed pubkey is upgradable, not APO",
        )

        internal_key = generate_privkey()
        utxo = self.fund_tapscript_v2(CScript([OP_1]), internal_key=internal_key)
        keypath_tx = CTransaction()
        keypath_tx.version = 2
        keypath_tx.vin = [CTxIn(COutPoint(int(utxo["txid"], 16), utxo["vout"]), CScript(), SEQUENCE_FINAL)]
        keypath_spk = bytes.fromhex(node.getaddressinfo(node.getnewaddress(address_type="bech32"))["scriptPubKey"])
        keypath_tx.vout = [CTxOut(utxo["amount"] - 10_000, keypath_spk)]
        spent_output = CTxOut(utxo["amount"], utxo["tap"].scriptPubKey)
        sighash = TaprootSignatureHash(keypath_tx, [spent_output], SIGHASH_DEFAULT, 0, scriptpath=False)
        tweaked_key = tweak_add_privkey(internal_key, utxo["tap"].tweak)
        keypath_tx.wit.vtxinwit = [CTxInWitness()]
        keypath_tx.wit.vtxinwit[0].scriptWitness.stack = [sign_schnorr(tweaked_key, sighash)]
        self.submit_and_mine(keypath_tx, "key-path spend")

        internal_key = generate_privkey()
        internal_pubkey = compute_xonly_pubkey(internal_key)[0]
        script_a = CScript([internal_pubkey, OP_CHECKSIG])
        script_b = CScript([OP_1])
        script_c = CScript([bytes([42]), bytes([42]), OP_EQUAL])
        utxo = self.fund_tapscript_v2(
            script_b,
            internal_key=internal_key,
            leaves=[
        ...
        self.submit_and_mine(self.spending_tx(utxo), "multi-leaf spend")

        key1 = generate_privkey()
        key2 = generate_privkey()
        pubkey1 = compute_xonly_pubkey(key1)[0]
        pubkey2 = compute_xonly_pubkey(key2)[0]
        multisig_script = CScript([pubkey1, OP_CHECKSIG, pubkey2, OP_CHECKSIGADD, OP_2, OP_NUMEQUAL])
        utxo = self.fund_tapscript_v2(multisig_script)
        spending_tx = self.spending_tx(utxo)
        sig1 = self.script_signature(utxo, spending_tx, key1)
        sig2 = self.script_signature(utxo, spending_tx, key2)
        spending_tx.wit.vtxinwit[0].scriptWitness.stack = [
            sig2,
            sig1,
            bytes(utxo["script"]),
        ...
        self.submit_and_mine(spending_tx, "2-of-2 CHECKSIGADD leaf")

        for hashtype, name in [
            (SIGHASH_DEFAULT, "SIGHASH_DEFAULT"),
            (SIGHASH_ALL, "SIGHASH_ALL"),
            (SIGHASH_NONE, "SIGHASH_NONE"),
            (SIGHASH_SINGLE, "SIGHASH_SINGLE"),
            (SIGHASH_ANYONECANPAY | SIGHASH_ALL, "SIGHASH_ALL|ANYONECANPAY"),
        ]:
            key = generate_privkey()
            pubkey = compute_xonly_pubkey(key)[0]
            sighash_script = CScript([pubkey, OP_CHECKSIG])
            utxo = self.fund_tapscript_v2(sighash_script)
            spending_tx = self.spending_tx(utxo)
            signature = self.script_signature(utxo, spending_tx, key, hashtype=hashtype)
            spending_tx.wit.vtxinwit[0].scriptWitness.stack.insert(0, signature)
            self.submit_and_mine(spending_tx, f"{name} leaf")

        cat_script = CScript([b"\x01\x02", b"\x03\x04", OP_CAT, b"\x01\x02\x03\x04", OP_EQUAL])
        utxo = self.fund_tapscript_v2(cat_script)
        self.submit_and_mine(self.spending_tx(utxo), "OP_CAT leaf")

        mul_script = CScript([OP_2, OP_3, OP_MUL, OP_6, OP_EQUAL])
        utxo = self.fund_tapscript_v2(mul_script)
        self.submit_and_mine(self.spending_tx(utxo), "OP_MUL leaf")
        ...
        merkle_script = CScript([OP_CAT, OP_SHA256, hash23, OP_CAT, OP_SHA256, root, OP_EQUAL])
        utxo = self.fund_tapscript_v2(merkle_script)
        spending_tx = self.spending_tx(utxo, witness_elements=[leaf0, leaf1])
        self.submit_and_mine(spending_tx, "OP_CAT merkle proof leaf")

        locktime = node.getblockcount()
        cltv_script = CScript([locktime, OP_CHECKLOCKTIMEVERIFY, OP_DROP, OP_1])
        utxo = self.fund_tapscript_v2(cltv_script)
        spending_tx = self.spending_tx(utxo, nlocktime=locktime, nsequence=0)
        self.submit_and_mine(spending_tx, "OP_CHECKLOCKTIMEVERIFY leaf")

        for name, success_op in (("OP_INTERNALKEY", OP_INTERNALKEY), ("OP_CHECKSIGFROMSTACK", OP_CHECKSIGFROMSTACK)):
    ...
    def test_locktime_sequence_val64(self):
        ...
        spenders = [
            make_lock_spender("v2/cltv_five_byte_zero", CScript([OP_CHECKLOCKTIMEVERIFY, OP_DROP, OP_1]), [b"\x00" * 5]),
            make_lock_spender("v2/cltv_too_large", CScript([OP_CHECKLOCKTIMEVERIFY, OP_DROP, OP_1]), [b"\x00\x00\x00\x00\x01"]),
            make_lock_spender("v2/csv_five_byte_zero", CScript([OP_CHECKSEQUENCEVERIFY, OP_DROP, OP_1]), [b"\x00" * 5]),
            make_lock_spender("v2/csv_unsatisfied_wide", CScript([OP_CHECKSEQUENCEVERIFY, OP_DROP, OP_1]), [b"\x02\x00\x00\x00\x00"]),
            make_lock_spender("v2/csv_uint32_max_disable", CScript([OP_CHECKSEQUENCEVERIFY, OP_DROP, OP_1]), [v2_num((1 << 32) - 1)]),
            make_lock_spender("v2/csv_overflow_disable", CScript([OP_CHECKSEQUENCEVERIFY, OP_DROP, OP_1]), [v2_num((1 << 32) | (1 << 31))]),
        ]
        ...
        def spend_one(index, *, nlocktime, nsequence, accept, err_msg=None):
            ...
            if accept:
                ...
                assert node.getmempoolentry(spend_tx.hash) is not None
            else:
    ...
    def test_transaction_wide_varops_budget(self):
        ...
        def make_spend_tx(funded_inputs):
            ...
            spend_tx.vout = [CTxOut(sum(output.nValue for _outpoint, output, _spender in funded_inputs) - 50_000, host_spk)]
        ...
        assert node.getmempoolentry(accepted_tx.hash) is not None
        self.block_submit(
            node,
            [accepted_tx],
    ...
    def test_final_success_varops_budget(self):
        ...
        host_spk = bytes.fromhex(node.getaddressinfo(node.getnewaddress(address_type="bech32"))["scriptPubKey"])
        ...
        script_cost = mul_cost(selected_operand_size, selected_operand_size) + upshift_cost(len(final_value), final_shift_bits)
        tx_budget = spend_tx.get_weight() * VAROPS_BUDGET_PER_WEIGHT
