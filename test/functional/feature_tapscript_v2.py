...
class TapscriptV2Test(BitcoinTestFramework):
    ...
    def test_op_checksig_basic(self):
        ...
        try:
            ...
            # Create spending transaction using script path
            spending_tx = self.create_tapscript_v2_spending_tx(utxo_info)
            
            # Create CTxOut for the spent output
            ...
            for hash_type, name, append_byte in test_cases:
                try:
                    ...
                    if result['allowed']:
                        self.nodes[0].sendrawtransaction(spending_tx.serialize().hex())
                        self.generate(self.nodes[0], 1)
                        self.log.info(f"  ✓ {name} (0x{hash_type:02x}) - {'64' if not append_byte else '65'} bytes")
                        passed += 1
                    else:
                ...
                try:
                    ...
                    if should_pass:
                        ...
                        if result['allowed']:
                            self.nodes[0].sendrawtransaction(spending_tx.serialize().hex())
                            self.generate(self.nodes[0], 1)
                            self.log.info(f"  ✓ {description} - PASSED (final size: {final_size} bytes)")
                            passed += 1
                        else:
                        ...
                        if not result['allowed']:
                            ...
                            if expected_rejection and expected_rejection in actual_reason:
                                self.log.info(f"  ✓ {description} - REJECTED correctly ({actual_reason})")
                                passed += 1
                            else:
                    ...
                    if should_pass:
                        ...
                        self.log.info(f"  ✓ {description} - REJECTED (exception: {str(e)[:50]}...)")
                        passed += 1
    ...
    def test_op_mul_too_large(self):
        ...
        try:
            ...
            if not result['allowed']:
                ...
                self.log.error(f"Transaction should have been rejected but was accepted!")
                self.print_test_status(test_name, is_start=False, success=False)
    ...
    def test_stack_size_limit(self):
        ...
        try:
            ...
            for num_elements, should_pass, description in test_cases:
                try:
                    # Push num_elements via witness
                    # Script verifies we can handle that many elements by checking depth
                    unlocking_script = [bytes([i % 256]) for i in range(num_elements)]
                    
                    # Simple script: 
                    # - OP_DEPTH gets stack depth (pushes it on stack, so now num_elements + 1 items)
                    # - Check depth == num_elements
                    # - Then we need to drop everything except the comparison result
                    # 
                    # Better approach: Use OP_2DROP repeatedly to clear the stack except result
                    # Stack: [elem0, elem1, ..., elemN-1]
                    # OP_DEPTH -> [elem0, ..., elemN-1, N]
                    # Push expected -> [elem0, ..., elemN-1, N, num_elements]  
                    # OP_EQUAL -> [elem0, ..., elemN-1, result]
                    # Now drop N-1 elements using OP_2DROP and OP_NIP
                    
                    # Actually simpler: Just drop all witness elements and push OP_1 (always succeeds)
                    # This tests that we could PUT num_elements on the stack
                    script_ops = []
                    # Drop all witness elements
                    ...
                    if should_pass:
                        ...
                        self.log.info(f"  ✓ {description} - correctly rejected (exception)")
                        passed += 1
    ...
    def test_stack_element_size_limit(self):
        ...
        try:
            ...
            # Bitcoin's 400KB standard transaction size limit. Instead, we test that we can
    ...
    def test_total_stack_size_limit(self):
        ...
        try:
            ...
            # Note: Limited by Bitcoin's 400KB transaction size limit for witness data
            test_cases = [
    ...
    def test_witness_element_size_limit(self):
        ...
        try:
            # NOTE: We can't directly test the 4MB limit because Bitcoin's 400KB
                ...
                try:
                    ...
                    if should_pass:
                        if result['allowed']:
                            ...
                            self.log.error(f"  ✗ {description} - rejected: {result.get('reject-reason', 'Unknown')}")
                            self.log.error(f"     This likely means TAPSCRIPT_V2 is using old 520-byte limit!")
                            failed += 1
                    else:
                        if not result['allowed']:
                            self.log.info(f"  ✓ {description} - correctly rejected")
                ...
                try:
                    ...
                    if should_pass:
                        if result['allowed']:
                            self.nodes[0].sendrawtransaction(spending_tx.serialize().hex())
                            self.generate(self.nodes[0], 1)
                            self.log.info(f"  ✓ {description} (final: {final_size:,} bytes)")
                            passed += 1
                        else:
                            self.log.error(f"  ✗ {description} - rejected: {result.get('reject-reason', 'Unknown')}")
                            failed += 1
                        ...
                        if not result['allowed']:
                            self.log.info(f"  ✓ {description} - correctly rejected (would be {final_size:,} bytes)")
                            passed += 1
                        else:
                            self.log.error(f"  ✗ {description} - should be rejected but was accepted!")
                            failed += 1
    ...
    def test_merkle_proof_verification(self):
        ...
        try:
            ...
            if result:
                self.log.info(f"  ✓ Successfully verified merkle proof on-chain")
                self.log.info(f"  ✓ Proved leaf0 ({leaf0.hex()[:16]}...) is in tree")
                self.log.info(f"  ✓ Root: {root.hex()[:32]}...")
                self.print_test_status(test_name, is_start=False, success=True)
            else:
                self.print_test_status(test_name, is_start=False, success=False)
    ...
    def test_varops_budget_signature_limit(self):
        """Test: Varops budget enforcement for signature operations
        
        This test validates that the varops budget correctly limits the number of
        signature operations in a transaction.
        
        Budget formula: transaction_weight * 5200
        Signature cost: 260,000 varops (5200 * 50)
        Max signatures: transaction_weight / 50
        
        We'll create transactions with different numbers of signatures and verify
        that transactions exceeding the budget are rejected.
        """
        test_name = "Varops budget: Signature operation limit"
        self.print_test_status(test_name, is_start=True)
        
        try:
            # We'll test with different numbers of required signatures
            # First, let's estimate a minimal transaction weight
            # A typical taproot spend is around 200-300 weight units for the base tx
            # Plus witness data (signatures, script, control block)
            # Each signature is ~64 bytes = 64 weight units
            # Script and control block add more weight
            
            # Strategy: Create transactions with N signatures and test the boundary
            
            # Test cases: (num_signatures, should_pass, description)
            
            # Each signature costs 260,000 varops but adds ~100 weight units,
            # which adds 520,000 varops budget. So signatures are SELF-FUNDING and we can't
            # exceed the budget just by adding more signatures!
            
            # To test budget enforcement, we need to verify the math is correct and that
            # the budget scales properly with transaction weight.
            test_cases = [
                (2, True, "2 signatures - well within budget"),
                (5, True, "5 signatures - within budget"),
                (10, True, "10 signatures - within budget"),
                (20, True, "20 signatures - within budget"),
                (30, True, "30 signatures - within budget"),
                (50, True, "50 signatures - still within budget (self-funding)"),
            ]
            
            passed = 0
            failed = 0
            
            for num_sigs, should_pass, description in test_cases:
                try:
                    ...
                    for i in range(num_sigs):
                        ...
                        keys.append(key)
                        pubkey_xonly, _ = compute_xonly_pubkey(key.get_bytes())
                        pubkeys_xonly.append(pubkey_xonly)
                    
                    # Build N-of-N multisig script using OP_CHECKSIGADD
                    # Pattern: <pubkey1> OP_CHECKSIG <pubkey2> OP_CHECKSIGADD ... <pubkeyN> OP_CHECKSIGADD <N> OP_EQUAL
                    script_ops = [pubkeys_xonly[0], OP_CHECKSIG]
                    for i in range(1, num_sigs):
                        script_ops.extend([pubkeys_xonly[i], OP_CHECKSIGADD])
                    # Check that we got exactly num_sigs valid signatures
                    # Use OP_N for numbers 1-16, minimal encoding otherwise
                    if num_sigs == 0:
                        expected_count = OP_0
                    elif num_sigs <= 16:
                        # Use OP_1 through OP_16
                        expected_count = CScriptOp(0x50 + num_sigs)  # OP_1 = 0x51, OP_2 = 0x52, etc.
                    else:
                        # For numbers > 16, use minimal encoding
                        expected_count = num_sigs.to_bytes((num_sigs.bit_length() + 7) // 8, 'little')
                    script_ops.extend([expected_count, OP_EQUAL])
                    ...
                    # Each insert(0) pushes to front, so we insert in order to get reversed stack
                    # If we have [sig1, sig2, sig3], we want witness [sig3, sig2, sig1, script, control]
                    # So stack during execution is [sig3, sig2, sig1] with sig1 on TOP
                    # This way first CHECKSIG uses sig1, second uses sig2, etc.
                    for sig in signatures:
                        spending_tx.wit.vtxinwit[0].scriptWitness.stack.insert(0, sig)
                    
                    # Calculate actual transaction weight and budget
                    tx_weight = spending_tx.get_weight()
                    varops_budget = tx_weight * 5200
                    varops_used = num_sigs * 260000
                    max_sigs_for_weight = tx_weight // 50
                    
                    self.log.info(f"\n  Testing {num_sigs} signatures:")
                    self.log.info(f"    Transaction weight: {tx_weight}")
                    self.log.info(f"    Varops budget: {varops_budget:,}")
                    self.log.info(f"    Varops used: {varops_used:,} ({num_sigs} sigs × 260,000)")
                    self.log.info(f"    Max signatures for this weight: {max_sigs_for_weight}")
                    self.log.info(f"    Budget remaining: {varops_budget - varops_used:,}")
                    
                    # Test transaction
                    result = self.nodes[0].testmempoolaccept([spending_tx.serialize().hex()], maxfeerate=0)[0]
                    
                    # Determine expected outcome based on actual calculation
                    if should_pass is None:
                        # Boundary case - determine based on actual budget
                    ...
                    if should_pass:
                        if result['allowed']:
                            self.nodes[0].sendrawtransaction(spending_tx.serialize().hex())
                            self.generate(self.nodes[0], 1)
                            self.log.info(f"  ✓ {description} - PASSED (within budget)")
                            passed += 1
                        else:
            ...
            # Summary
            self.log.info(f"\nVarops signature budget tests: {passed} passed, {failed} failed")
            
            if failed > 0:
                self.print_test_status(test_name, is_start=False, success=False)
                raise AssertionError(f"{failed} varops signature budget test(s) failed")
            else:
                self.print_test_status(test_name, is_start=False, success=True)
        ...
        try:
            ...
            for num_verifications, should_pass, description in test_cases:
                try:
                    ...
                    # Calculate transaction weight and budget
                    tx_weight = spending_tx.get_weight()
                    varops_budget = tx_weight * 5200
                    ...
                    self.log.info(f"    Transaction weight: {tx_weight}")
                    self.log.info(f"    Varops budget: {varops_budget:,}")
                    self.log.info(f"    Varops used: {varops_used:,} ({num_verifications} CHECKSIGs × 260,000)")
                    self.log.info(f"    Max verifications for this weight: {max_verifications_for_weight}")
                    self.log.info(f"    Budget remaining: {varops_budget - varops_used:,}")
    ...
    def run_test(self):
        # Generate initial blocks and fund wallet
        ...
        # New boundary tests for TAPSCRIPT_V2 limits
        ...
        # Varops budget tests
        self.test_varops_budget_signature_limit()
    ...
    def test_simple_tapscript_v2_transaction(self, locking_script, unlocking_script) -> bool:
        ...
        try:
            utxo_info = self.create_tapscript_v2_funding_tx(locking_script, amount=0.1)
            # self.log.info(f"✓ Created TAPSCRIPT_V2 funding transaction: {utxo_info['funding_txid']}")
            spending_tx = self.create_tapscript_v2_spending_tx(utxo_info, extra_witness_elements=unlocking_script)
            # self.log.info(f"✓ Created TAPSCRIPT_V2 spending transaction: {spending_tx.txid_hex}")

            # # print tx size 
            # self.log.info(f"Tx size: {len(spending_tx.serialize())} B")
            # # print tx weight
            # self.log.info(f"Tx weight: {spending_tx.get_weight()} wu")
            # # print varops budget
            ...
            if result['allowed']:
                # self.log.info(f"✓ TAPSCRIPT_V2 transaction accepted")

                # broadcast transaction
                self.nodes[0].sendrawtransaction(spending_tx.serialize().hex())
                # self.log.info(f"✓ TAPSCRIPT_V2 transaction broadcasted")
                self.generate(self.nodes[0], 1)

                # wait for mempool to clear
                self.generate(self.nodes[0], 1)
                # self.log.info(f"✓ TAPSCRIPT_V2 transaction mempool cleared")
                self.generate(self.nodes[0], 1)
                # self.log.info(f"✓ TAPSCRIPT_V2 transaction mined")
            # else:
                # self.log.info(f"✗ TAPSCRIPT_V2 transaction rejected: {result.get('reject-reason', 'Unknown reason')}")
                # print stack top
                # self.log.info(f"Stack top: {spending_tx.wit.vtxinwit[0].scriptWitness.stack[-1].hex()}")
