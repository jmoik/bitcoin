...
class PSBTTest(BitcoinTestFramework):
    ...
    def test_finalized_ctv_psbt_precomputation(self):
        ...
        def make_psbt(commit_final_script_sigs):
            ...
            finalized_tx.vin = [CTxIn(scriptSig=final_script_sig), CTxIn(scriptSig=final_script_sig)]
            ...
            if commit_final_script_sigs:
                ...
                for txin in finalized_tx.vin:
                    txin.scriptSig = b""
                template_hash = finalized_tx.get_standard_template_hash(0)
                for txin in finalized_tx.vin:
                    txin.scriptSig = final_script_sig

            ctv_script = CScript([OP_DROP, template_hash, OP_CHECKTEMPLATEVERIFY, OP_DROP, OP_TRUE])
            anyone_can_spend_script = CScript([OP_DROP, OP_TRUE])

            def make_prev_tx(script_pub_key, marker):
                prev_tx = CTransaction()
                prev_tx.version = 2
                prev_tx.vin = [CTxIn(COutPoint(hash=int("11" * 32, 16), n=marker), CScript([OP_TRUE]))]
                prev_tx.vout = [CTxOut(nValue=100_000, scriptPubKey=script_pub_key)]
                prev_tx.rehash()
                return prev_tx

            prev_txs = [make_prev_tx(ctv_script, 0), make_prev_tx(anyone_can_spend_script, 1)]
            for index, prev_tx in enumerate(prev_txs):
                finalized_tx.vin[index].prevout = COutPoint(hash=int(prev_tx.hash, 16), n=0)
            finalized_tx.rehash()

            unsigned_tx = CTransaction()
            unsigned_tx.version = finalized_tx.version
            unsigned_tx.vin = [
                CTxIn(COutPoint(hash=txin.prevout.hash, n=txin.prevout.n), nSequence=txin.nSequence)
                for txin in finalized_tx.vin
            ]
        ...
        self.log.info("Valid finalized CTV PSBT analysis: %s", analysis)
        ...
        processed = node.descriptorprocesspsbt(valid_psbt, [])
        assert_equal(processed["complete"], True)
        assert_equal(processed["hex"], valid_hex)

        wallet_processed = wallet.walletprocesspsbt(valid_psbt, sign=False)
        assert_equal(wallet_processed["complete"], True)
        assert_equal(wallet_processed["hex"], valid_hex)
        ...
        assert_equal(
            analysis["error"],
            "PSBT is not valid. Finalized transaction failed script verification: Script failed an OP_CHECKTEMPLATEVERIFY operation",
        )
        ...
        processed = node.descriptorprocesspsbt(invalid_psbt, [])
        assert_equal(processed["complete"], False)
        assert "hex" not in processed

        wallet_processed = wallet.walletprocesspsbt(invalid_psbt, sign=False)
        assert_equal(wallet_processed["complete"], False)
        assert "hex" not in wallet_processed
